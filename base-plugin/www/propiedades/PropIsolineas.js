class PropIsolineas extends ZCustomController {
    onThis_init(options) {      
        this.codigo = "props";  
    ...
    refresca() {
        ...
        this.titulo.text = "Propieades Isolineas";
        this.edLineWidth.value = this.visualizador.lineWidth;
        this.edLineColor.value = this.visualizador.lineColor;
    ...
    onEdLineWidth_change() {
        let w = parseFloat(this.edLineWidth.value);
        if (!isNaN(w)) this.visualizador.lineWidth = w;
    }
    onEdLineColor_change() {
        let c = this.edLineColor.value;
        if (c) this.visualizador.lineColor = c;
    }
