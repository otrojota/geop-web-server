class WFiltrosMinZ extends ZDialog {
    onThis_init(options) {
        this.caret.hide();
        this.consulta = ConsultaGeoportal.creaClonDe(options.consulta);
    ...
    async clickNodo(nodo) {
        ...
        try {
            let rows = [];
            let n = await window.minz.cuentaValores(nodo.clasificador.dimensionCode);
            let dimVal = await window.minz.getValores(nodo.clasificador.dimensionCode, null, null, 0, (n > 50?50:n));
            rows.push({
                code:"sel-filas",
                icon:"fas fa-list",
                label:"Seleccionar [" + n + "]",
            ...
            */
            ...
            this.zpop = new ZPop(this.caret.view, rows, {
                ...
                onSearch:async textFilter => {
                    ...
                    let n = await window.minz.cuentaValores(nodo.clasificador.dimensionCode, textFilter);
                    console.log("n", n);
                    if (!n) return [{
                        tipo:"warng",
                        icon:"fas fa-exclamation-triangle",
                    ...
                    let dimVal = await window.minz.getValores(nodo.clasificador.dimensionCode, textFilter, null, 0, (n > 50?50:n));
